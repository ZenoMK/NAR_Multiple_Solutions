# THIS IS MOSTLY A COPY OF permute_everything, just with most things commented out, so it only shuffles `pos`
import pandas as pd
import time
import clrs
import jax
...
# -----------------------------------------------------------------------------------------------------------------
def permute_something_then_predict(ffs, predict_fn, num_perms, graph_num, new_rng_key, extras, dont_permute):
...
def permute_and_eval(sampler, predict_fn, sample_count, rng_key, extras, num_perms=1, dont_permute=False):
  ...
  while processed_samples < sample_count:  # do another batch
    ...
    batch_IDs, batch_As, batch_perms, batch_preds, batch_Ss = permute_something_then_predict(ffs=feedback.features,
...
if __name__ == '__main__':
  ...
  print(
    '================================================')  # FIXME: note: dedup_variety has no meaning if you dont permute the adjacency matri
