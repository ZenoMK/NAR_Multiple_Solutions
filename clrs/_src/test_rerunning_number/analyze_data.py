...
"""
print("here")
means20_50, std20_50 = generate_data(graphs_per_size=5, num_solutions=[20,50], min_size=min_size, max_size=max_size)
print("here1")
plt.plot([i for i in range(min_size, max_size + 1)], means20_50, label = "20 vs 50 re-runs", color = "red")
plt.fill_between(x = [i for i in range(min_size, max_size + 1)], y1 = means20_50 + std20_50, y2 = means20_50 - std20_50, alpha = 0.3)

means50_100, std50_100 = generate_data(graphs_per_size=10, num_solutions=[50,100],min_size=min_size, max_size=max_size)
plt.plot([i for i in range(min_size, max_size + 1)], means50_100, label = "50 vs 100 re-runs",color = "green")
plt.fill_between(x = [i for i in range(min_size, max_size + 1)], y1 = means50_100 + std50_100, y2 = means50_100 - std50_100, alpha = 0.3)
print("here2")
means20_100, std20_100 = generate_data(graphs_per_size=10, num_solutions=[20,100], min_size=min_size, max_size=max_size)
plt.plot([i for i in range(min_size, max_size + 1)], means20_100, label = "20 vs 100 re-runs",color = "blue")
...
plt.xlabel("Graph sizes")
plt.ylabel("KL-Divergence")
plt.title("KL-div between distributions from varying numbers of BF re-runs")
